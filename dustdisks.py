...
furnsh(spice_dir + 'mars.tm') # minimal kernel only for observer/instrument independent calculations
...
class DustParticle(Regolith):
	r = Float(2.5e-6)               # radius of the dust
    S = Property(depends_on 'r')    # surface of dust disk
    normal = Tuple
    gamma = Float
    sun_vec = Tuple     # direction to the sun as vector from the surface point in Mars coords frame
    Sc = Float # the solar constant the dust particle is exposed to
    mu = Property(depends_on = 'sun_vec') # cosine of sep vector between normal and sun
    # insolation [W/m**2]
    I = Property(depends_on = ['mu','Sc'])
    # Power [W = J/s]
    P = Property(depends_on = 'I')
    def __init__(self, normal, degree=None, axis=None):
        if degree not None and axis not None:
            rot_axis = zeros(3)
            rot_axis[axis] = 1
    ...
    def _get_S(self):
        ...
        return 0.5*tau*r*r
    def _get_mu(self):
        return cos(vsep(self.sun_vec, self.normal))
    def _get_I(self):
        return self.Sc*(1-self.A)*self.mu
    def _get_P(self):
...
while mspicer.l_s < 250:
    # adapt available energy on dust disks
    disk1.set(sun_vec=mspicer.sun_direction,Sc=mspicer.solar_constant)
    disk2.set(sun_vec=mspicer.sun_direction,Sc=mspicer.solar_constant)

    # degree of sun's culmination (highest point in the sky)
    hv =  tau/4 - mspicer.coords.lat + arcsin(sin(Mars.obliquity) * sin(deg2rad(-mspicer.l_s)))
    # lowest point of sun
    hn = -tau/4 + mspicer.coords.lat + arcsin(sin(Mars.obliquity) * sin(deg2rad(-mspicer.l_s)))
