...
class SourceFunction:
    ...
    def __add__(self, other): 
        return CompositeSourceFunction(self, other, 'add') # __radd__ not needed
...
class BlackBody(SourceFunction): 
    ...
    def __call__(self, wave): 
        BBCONST = 6.625e-34 * 3e8 / (1.38e-23 * 1e-10) 
...
class Gaussian(SourceFunction): 
    def __init__(self, center, width, amp=1):
        self.width = width
        self.center = center
        self.amp = amp
    def __call__(self, wave):
        return self.amp * n.exp(-(wave - self.center) ** 2 / (2 * self.width ** 2))
