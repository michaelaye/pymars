...
from numpy import array, cos, deg2rad, pi, sin, tan, zeros
from spiceypy import furnsh, vrotv, vsep
from traitlets import Float, HasTraits, Tuple

from spicer import MarsSpicer
...
class Mars(HasTraits):
    ...
    obliquity = Float(
        deg2rad(25.19)
    )  # degrees - angle of inclination of the axis of rotation to the orbital plane
...
class CO2_block(CO2, MarsSpicer):
    ...
    from little_helpers.CO2_phase_diagram import CO2_sublime_mass
...
class DustParticle(Regolith, MarsSpicer):
    ...
    def __init__(
        self, normal, degree=None, axis=None, time=None, rad=None, height=None
    ):
        MarsSpicer.__init__(self, time)
        self.r = rad
        self._h = height
    ...
    @property
    def h(self):
        h_CO2 = read_daily_average_CO2_depth(
            self.coords.dlat, self.coords.dlon, self.l_s
        )
        print(h_CO2)
        if self._h > h_CO2:
            return h_CO2
...
def main():
    ...
    while mspicer.l_s < 250:
        ...
        ds = cos(pi / 2.0 - h) * (x + r) * 1000000
        # put' sveta vo ldu v micronah
        ...
        mchd = rho * pi * r * r * r / 10.0  # why would this be the mass of the disk?
        ...
        if Tch < 217:
            ...
            tnagd = floor((disk1.S * disk1.r / 10.0) * disk1.sreg * dT / Ed1) + 1
            Tch = 217
        else:
            tnagd = 0
